...
13. 각 페이지에서 포스팅의 작성 시간대를 UI 텍스트로 보여주는 것이 필요하다.물론 개발 서버에서는 빌드와 렌더링 모두 로컬 환경에서 이루어지기 때문에 UI가 정확하게 일치하지만, 실제 배포는 netlify, vercel과 같은 Faas 환경에서 이루어지기 때문에 서버 시간과 클라이언트 기기가 렌더링하면서 인식하는 시간대가 다르다. 따라서 해당 시간 UI의 렌더링을 서버사이드에서 만들어두기 보다, useState와 useEffect 훅을 이용하여 클라이언트 사이드에서 이루어지도록 함(해결 완료) => 하지만 리-렌더링이 이루어지므로 동적으로 하는 방식도 고민해보자(블로그 참조)

14. 웹 크롤러에게 도움을 주기 위해서 siteMap.xml을 플러그인을 사용하여 작성해 놓을 필요가 있음
